...
const App: React.FC = () => {
  ...
  const handleCalculate = (inputs: CalculatorInputs) => {
    // 1. Realiza o cálculo local (física/química)
    const calcResult = calculateSequestration(inputs);
    setResult(calcResult);

    // 2. Se autorizado, envia para o banco de dados (Fire and forget)
    // Não usamos 'await' aqui para não travar a UI enquanto salva
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-emerald-100">
             ...
             <div className="sticky top-24 space-y-6">
                ...
                <div className="p-5 bg-white rounded-xl border border-slate-200 text-xs text-slate-500 leading-relaxed shadow-sm">
                  ...
                  <p className="mb-2 text-justify">
                    Esta ferramenta estima o sequestro de CO₂ no solo a partir da aplicação de biochar, utilizando como base o modelo proposto por Woolf et al. (2021), descrito no artigo Greenhouse Gas Inventory Model for Biochar Additions to Soil (Environ. Sci. Technol., 55:14795–14805, 2021. DOI: <a href="https://doi.org/10.1021/ACS.EST.1C02425" target="_blank" rel="noreferrer" className="text-emerald-600 hover:underline">10.1021/ACS.EST.1C02425</a>).
                  </p>
      ...
      <footer className="bg-white border-t border-slate-200 mt-12 py-8 no-print">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-xs text-slate-400 mt-2">© 2025 BioC-Calc. Todos os direitos reservados.</p>
        </div>
      </footer>
    </div>
  );
